...
rem ############################################################################
rem NAME
rem        exifCopyRightsUpdate - Write copyrights information in files
rem
rem SYNTAX
rem        exifCopyRightsUpdate -A AUTHOR FILE...
rem
rem DESCRIPTION
rem        Pass the arguments to the provided bash script
rem        in order to add copyrights information.
rem
rem ARGUMENTS
rem        -A AUTHOR    The name of the person who created the image
...
if "%~1"=="-A" goto set_Author
rem else
if not defined Photographer goto exit_syntax_error
goto exifUpdate


:set_Author
rem Set Author argument
shift
if "%~1"=="" goto end_arg_parse
set "Photographer=-Photographer=%~1"
goto next_arg
...
bash ./exifCopyRightsUpdate "%Photographer%" "%nixFilePath%"
...
:exit_syntax_error
echo SYNTAX: exifCopyRightsUpdate -A AUTHOR FILE..
pause
exit /B 1
